...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${listing.name} — ${listing.category} in ${listing.city}, FL`;
  const description =
    listing.shortDescription ||
    listing.description?.slice(0, 160) ||
    `${listing.name} is a ${listing.category} shop located in ${listing.city}, FL.`;
...
export default async function ListingPage({ params }: Props) {
  ...
  const relatedListings = cityListings
    .filter((l) => l.slug !== listing.slug)
    .slice(0, 4);
  const cityGuides = seoPages
    .filter((p) => p.city === listing.city)
    .slice(0, 4);
      ...
      <div className="container py-4 sm:py-8">
        ...
        <div className="mt-4 sm:mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          <aside className="space-y-4">
            ...
            <div className="card p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-gray-800">
                  {listing.address}
                  <br />
                  <Link href={citySlug} className="text-amber-600 hover:underline">
                    {listing.city}
                  </Link>
                  , FL
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {listing.instagram && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-pink-50 text-pink-700 text-sm rounded-lg hover:bg-pink-100 transition-colors"
                        title="Instagram"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {listing.youtube && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 text-sm rounded-lg hover:bg-red-100 transition-colors"
                        title="YouTube"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              ...
              {/* Request a Quote CTA (scrolls to inline form below) */}
              <a
                href="#quote-form"
            ...
            {/* Inline Quote Form */}
            <div id="quote-form" className="card p-6">
              <h3 className="text-base font-semibold text-gray-900 mb-4">
                Request a Quote from {listing.name}
              </h3>
              <QuoteForm compact shopName={listing.name} />
            </div>
          </aside>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {relatedListings.map((rl) => (
              ...
              >
                ...
                {rl.phone && (
                  <p className="text-xs text-amber-600 mt-1">{rl.phone}</p>
                )}
              </Link>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {cityGuides.map((guide) => (
              <Link
                ...
                className="block p-4 bg-gray-50 border border-gray-200 rounded-lg hover:border-amber-300 hover:bg-white transition-all"
              >
                <h3 className="text-sm font-semibold text-gray-900 line-clamp-2">
                  {guide.title}
                </h3>
      ...
      {listing.phone && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 lg:hidden z-40">
          <div className="flex gap-2 max-w-lg mx-auto">
            <a
              href="#quote-form"
              className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-amber-400 text-slate-900 font-semibold rounded-lg hover:bg-amber-300 transition-colors"
            >
              Get Quote
            </a>
            <a
              href={`tel:${listing.phone}`}
              className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 text-white font-medium rounded-lg hover:bg-slate-800 transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="none"
