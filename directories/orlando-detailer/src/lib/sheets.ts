import { Listing, Category, SEOPage, BlogPost, QuoteLead, CityGroup, StateGroup } from "./types";
...
function mapListing(row: SheetRow): Listing {
  return {
    ...
    brandsUsed: parseList(row["Brands Used"]),
    quoteAccepted: row["Quote Accepted"] ? parseBoolean(row["Quote Accepted"]) : undefined,
...
let cachedWriteToken: { token: string; expires: number } | null = null;

async function getWriteAccessToken(): Promise<string> {
  if (cachedWriteToken && Date.now() < cachedWriteToken.expires) {
  ...
  if (!res.ok) {
    throw new Error(`Google auth (write) error: ${res.status} ${await res.text()}`);
  }
...
const QUOTE_LEADS_SHEET = process.env.SHEETS_QUOTE_LEADS_TAB || "Quote Leads";

export async function submitQuoteLead(lead: Omit<QuoteLead, "id">): Promise<void> {
  const token = await getWriteAccessToken();
  const range = encodeURIComponent(QUOTE_LEADS_SHEET);
  ...
  const row = [
    ...
    lead.serviceType,               // Service Type
    lead.subcategory || "",         // Subcategory
    lead.vehicleYear || "",         // Vehicle Year
    ...
    lead.source,                    // Source
    (lead.notifiedShops || []).join(", "), // Notified Shops
    lead.status,                    // Status
  ];
  ...
  if (!res.ok) {
    throw new Error(`Quote lead submission error: ${res.status} ${await res.text()}`);
  }
}
